...
> **Note:** `.github/copilot-instructions.md` still describes an earlier PostgreSQL/Npgsql design (including a `snake_case` naming convention) that is no longer accurate — the project has since moved to SQL Server with default (Pascal/camel-case-ish) EF Core naming. `README.md` was updated alongside this file and reflects the current architecture; trust the code (`ServiceCollectionExtensions.cs`, `.env.example`, `compose.yaml`) over `copilot-instructions.md` for connection/database details.
